...
func (cs *CoinSet) TotalValue() (value qtumutil.Amount) {
...
func satisfiesTargetValue(targetValue, minChange, totalValue qtumutil.Amount) bool {
...
type CoinSelector interface {
	CoinSelect(targetValue qtumutil.Amount, coins []Coin) (Coins, error)
}
...
type MinIndexCoinSelector struct {
	MaxInputs       int
	MinChangeAmount qtumutil.Amount
}

// CoinSelect will attempt to select coins using the algorithm described
// in the MinIndexCoinSelector struct.
func (s MinIndexCoinSelector) CoinSelect(targetValue qtumutil.Amount, coins []Coin) (Coins, error) {
	cs := NewCoinSet(nil)
...
func (s MinNumberCoinSelector) CoinSelect(targetValue qtumutil.Amount, coins []Coin) (Coins, error) {
...
func (s MaxValueAgeCoinSelector) CoinSelect(targetValue qtumutil.Amount, coins []Coin) (Coins, error) {
	sortedCoins := make([]Coin, 0, len(coins))
	sortedCoins = append(sortedCoins, coins...)
...
func (s MinPriorityCoinSelector) CoinSelect(targetValue qtumutil.Amount, coins []Coin) (Coins, error) {
...
// qtumutil.Tx, a specific outpoint index, and the number of confirmations
...
func (c *SimpleCoin) Value() qtumutil.Amount {
	return qtumutil.Amount(c.txOut().Value)
}
